...
fn env() -> Vec<(String, Type)> {
    ...
    */
    vec![
        (
            "S".to_string(),
            Type::ProdType(vec![("a".to_string(), Type::TypeEnvRef("A".to_string()))]),
        ),
        (
            "SA".to_string(),
...
fn test_lift_part1() {
    let env = &env();
    let v = &vec![("a".to_string(), Type::TypeEnvRef("A".to_string()))];
    let derive = &Type::ProdType(vec![("a".to_string(), Type::TypeEnvRef("A".to_string()))]);
